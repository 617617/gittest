# Codex Test Result: active_lease_other_holder

Outcome: BLOCK
SelfEvaluation: PASS
ChatCommandReceived: - (none; bk sync said Do not run /bk work)
ExpectedFromSection8: `active_lease_other_holder` | Print failure code; do not act
...
SCENARIO: active_lease_other_holder
------------------------------------------------------------------------
NEXT: Do not run /bk work
Task: blue-k/k1
Holder: Codex (blue-k-main-runner) active lease; progress=pending
Last: origin/blue-k/k1@1111111
ChatTarget: -
ChatCommand: -
HERE: BK_ROLE=codex
WHY: another holder has an active coordination lease
UNIT: blue-k-main-runner will select/resume from MAIN_PACKAGE_PROGRESS.md
LOCK: active lease held by codex/machine-b
SAFE_POINT: origin/blue-k/k1@1111111
STOP_IF: blocked until failure is resolved
FailureCode: ACTIVE_LEASE_OTHER_HOLDER
...
self-check:
  ...
  local HEAD:          1111111
  worktree clean:      yes
  competing lease:     active lease held by codex/machine-b
  atomic push:         unknown
  audit verdict:       unknown
  consensus state:     none

ACTIVE_LEASE_OTHER_HOLDER
BLOCK: the competing lease row is active for another holder, which maps to ACTIVE_LEASE_OTHER_HOLDER.
```
